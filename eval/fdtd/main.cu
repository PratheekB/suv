...
void fdtdCuda(DATA_TYPE* _fict_gpu, DATA_TYPE* ex_gpu, DATA_TYPE* ey_gpu, DATA_TYPE* hz_gpu)
{
  ...
  /*     cudaMemPrefetchAsync(ex_gpu, sizeof(DATA_TYPE) * NX * (NY+1), GPU_DEVICE, 0 ); */
  /*     available -= sizeof(DATA_TYPE) * NX * (NY+1); */
  /*   } else { */
  /*     cudaMemPrefetchAsync(ex_gpu, available, GPU_DEVICE, 0 ); */
  /*     available = 0; */
  /*   } */
  /*   if(available > sizeof(DATA_TYPE) * NY * (NX+1)) { */
  /*     cudaMemPrefetchAsync(ey_gpu, sizeof(DATA_TYPE) * NY * (NX+1), GPU_DEVICE, 0 ); */
  /*     available -= sizeof(DATA_TYPE) * NY * (NX+1); */
  /*   } else { */
  /*     cudaMemPrefetchAsync(ey_gpu, available, GPU_DEVICE, 0 ); */
  /*     available = 0; */
  /*   } */
  /*   if(available > sizeof(DATA_TYPE) * NY * (NX)) { */
  /*     cudaMemPrefetchAsync(hz_gpu, sizeof(DATA_TYPE) * NY * (NX), GPU_DEVICE, 0 ); */
  /*     available -= sizeof(DATA_TYPE) * NY * (NX); */
  /*   } else { */
  /*     cudaMemPrefetchAsync(hz_gpu, available, GPU_DEVICE, 0 ); */
  /*     available = 0; */
  /*   } */
  /*   cudaMemAdvise(ex_gpu, sizeof(DATA_TYPE) * NX*(NY+1), cudaMemAdviseSetAccessedBy, 0); */
  /*   cudaMemAdvise(ey_gpu, sizeof(DATA_TYPE) * (NX+1)*(NY), cudaMemAdviseSetAccessedBy, 0); */
  /*   cudaMemAdvise(hz_gpu, sizeof(DATA_TYPE) * (NX)*(NY), cudaMemAdviseSetAccessedBy, 0); */
  /* } */
