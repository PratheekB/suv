                           ...
                           int mype, double *kernel_time)
{
 ...
 nvml_start();
  ...
  /* penguinSetPrioritizedLocation(d_b , sizeof(int) * N * N, 0); */
      /* cudaMemPrefetchAsync(index_grid ,sizeof(int) * (unsigned long long)SD.length_index_grid, 0,  0); */

      /* cudaMemPrefetchAsync(concs_d ,sizeof(double) * (unsigned long long)SD.length_concs, 0,  0); */
      /* cudaMemPrefetchAsync(mats_d ,sizeof(double) * (unsigned long long)SD.length_mats, 0,  0); */
      /* cudaMemPrefetchAsync(nuclide_grid_d ,sizeof(NuclideGridPoint) * (unsigned long long)SD.length_nuclide_grid, 0,  0); */
      /* cudaMemPrefetchAsync(index_grid_d ,sizeof(int) * (unsigned long long)SD.length_index_grid, 0,  0); */
      /*   cudaMemAdvise(verification_d, sizeof(int) * in.lookups, cudaMemAdviseSetPreferredLocation, 0); */
